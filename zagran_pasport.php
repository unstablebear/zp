...
TBL;
    ...
    $monthes = array ( "01" => "января", "02" => "февраля", "03" => "марта", "04" => "апреля", "05" => "мая", "06" => "июня", "07" => "июля",
    ...
    $pdf->SetFont('dejavusans', 'B', 9);
    $pdf->Text(14, $y + 47, iconv("cp1251", "utf-8", 'Я предупрежден, что сообщение ложных сведений в заявлении или представление поддельных'), 0);
    $pdf->Text(14, $y + 51, iconv("cp1251", "utf-8", 'документов влечет ответственность, установленную законодательством.'), 0);

    $pdf->SetFont('dejavusans', '', 9);
    $pdf->SetXY(14, $y + 54);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", '"________" ____________ 20      года   __________________________'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 61);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", 'Дата приема документов "________" ____________ 20      года'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 68);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", 'Регистрационный номер ___________________________________'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 75);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", 'Подпись, фамилия сотрудника, принявшего заявление'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 80);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", '  _____________________________________________________________'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 87);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", 'Выдан паспорт серии ______________ номер  ________________'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 97);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", '"________" ____________ 20      года   (дата выдачи)'), 0, 1, 'L');

    $pdf->SetXY(14, $y + 111);
    $pdf->Cell(0, 0, iconv("cp1251", "utf-8", 'Паспорт получил ___________________ (подпись заявителя)'), 0, 1, 'L');
